...
from tor.core.helpers import clean_id
...
def process_command(reply, config):
    ...
    with open("commands.json", newline="") as commands_file:
        ...
        if not allowed_execute_command(command, reply, config):
            logging.info(
                f"{reply.author.name} failed to run {requested_command}, because they aren't a mod, or aren't whitelisted to use this command"
            )
